...
import { VideoTutorialWidget } from '@/components/video-tutorials-preview/VideoTutorialWidget';
import { VideoTutorialsConfig } from '@/components/VideoTutorialsBuilder';

interface VideoTutorialsPreviewProps {
  configs: VideoTutorialsConfig[];
}

export const VideoTutorialsPreview = ({ configs }: VideoTutorialsPreviewProps) => {
  return (
    <div className="h-full bg-gray-100 relative overflow-hidden">
      ...
      {configs.map((config) => (
        <VideoTutorialWidget key={config.id} config={config} />
      ))}
    </div>
  );
};
